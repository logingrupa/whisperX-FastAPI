...
<task type="auto">
  ...
  <action>
    ...
    Constructor: `__init__(self, repository: ITaskRepository)` -- stores the repository for task persistence.

    Create an async method with this exact signature:
    ```python
    async def start_transcription(self, file_path: str, metadata: dict, background_tasks: BackgroundTasks) -> str
    ```

    The method must:
    1. Accept a completed file path (str from tuspyserver) and metadata dict (from TUS client)
    2. Validate the assembled file using `validate_magic_bytes` from `app.infrastructure.storage.magic_validator` - pass the file path and derive extension from metadata `filename` key. If invalid, raise `ValueError("Invalid file type: magic bytes validation failed")`.
    ...
    7. Schedule transcription as a background task: `background_tasks.add_task(process_audio_common, params)` -- process_audio_common accepts `SpeechToTextProcessingParams` as its first argument; all optional service dependencies (whisperx_service, alignment_service, etc.) default to None internally, which triggers WhisperX defaults. Do NOT pass explicit service instances.
    8. Return the task identifier string

    CRITICAL: The hook must NOT block. Transcription runs in background. The method returns quickly after scheduling.

    Error handling strategy -- raise exceptions, do NOT silently swallow:
    - Wrap the entire method body in try/except
    - If validation fails (step 2), the raised ValueError propagates up
    - If audio loading or task creation fails, log the error at ERROR level and re-raise the exception
    - tuspyserver will catch the exception and report the failure to the client
    - This ensures the user knows transcription failed rather than silently losing their upload
  ...
  <done>
    UploadSessionService is importable, has an async `start_transcription(self, file_path: str, metadata: dict, background_tasks: BackgroundTasks) -> str` method that validates the file, creates a task, and schedules process_audio_common as a background task.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    2. Create the DI factory function `create_upload_complete_hook`:
       ...
       - Inside: instantiate `service = UploadSessionService(repository)`
       - Return an async handler function that calls `await service.start_transcription(file_path, metadata, background_tasks)`
  ...
  <done>
    TUS router's upload_complete_dep is wired to create_upload_complete_hook. When a TUS upload completes, the hook validates the file, creates a task, and schedules background transcription by calling service.start_transcription(file_path, metadata, background_tasks).
  </done>
</task>
